...
labeling:
  ...
  tp_atr_mult:                     # PER-ASSET dict (Decision v2.64 schema; values per v2.65 widening correction)
    ...
    SOL: 2.5                       # DR-012: 2.2→2.5 widening per Decision v2.65 (user-correction of v2.64 misjudgment direction; alt barriers WIDER than BTC, not tighter)
...
| Path D sixth — per-asset tp/sl experiment (WIDEN correction) | **v2.65 (DR-012)** | **BTC=2.4 / SOL=2.5 / LINK=2.5** | **0.275% BTC / 0.60% SOL+LINK** | **12** | (BTC unchanged) | (BTC unchanged) | **Active for SOL+LINK; user-correction of v2.64 misjudgment — alts WIDENED 2.2→2.5 not tightened. BTC frozen at v2.61 PASS — DO NOT RE-RUN** |
| Path D seventh candidate — further widen (if more buffer wanted) | DR-013 candidate (extend) | BTC=2.4 / SOL=2.7 / LINK=2.7 | (current) | 12 | — | — | **User-staged plan 2026-05-04: 'test 2.5 first and then 2.7'.** Triggers if v2.65 lands in band but user wants more NEUTRAL buffer above 15% floor. Per v2.64 alt step-response (−0.50pp/0.1 ATR), 2.5→2.7 projects +1.0pp NEUTRAL → ~17.0% / ~17.25% on SOL / LINK |
| Fallback if v2.65 over-widens | DR-013 candidate (revert) | revert SOL/LINK to 2.4 | (asset-specific) | 12 | — | — | Triggers if NEUTRAL >25% on alts post-2.5 (unlikely per ~16% projection) |
| Phase 1.14 freeze candidate (if v2.65 OR v2.66-2.7 PASSES) | DR-014 candidate (commit) | BTC=2.4 / SOL ∈ {2.5, 2.7} / LINK ∈ {2.5, 2.7} | (current) | 12 | (current) | (current) | Triggers when user accepts the locked alt tp/sl value; freezes feature matrix + label parquet for Phase 2 training |
...
- v2.65 (DR-012) is the WIDENING CORRECTION of v2.64. User identified misjudgment 2026-05-04: original v2.64 hypothesis was that 2.4 was "wide for SOL/LINK" → tested narrower 2.2. Experiment results showed direction was wrong: tightening drove NEUTRAL further below floor. **Correction**: alt tp/sl 2.2 → 2.5 (WIDENING, not tightening). User professional intent: alts have higher ATR per bar than BTC, so 2.5×ATR for alts ends up at a similar or larger absolute % move target than BTC's 2.4×ATR; alts SHOULD have wider barriers in ATR-multiplier units to give moves room to develop. Empirical projection (using v2.64-calibrated alt step-response): SOL NEUTRAL 14.54% → ~16.04%, LINK 14.75% → ~16.25%. Both projected to land cleanly in [15,25] band with comfortable margin (>1pp above 15% floor). Better than v2.62 (which had SOL=15.55%, LINK=15.75% just above floor). LONG/SHORT each drop ~0.75-1.0pp into ~41-42% range (well within [35,45] band). Chop UNCHANGED (validated invariant from v2.64). If v2.65 PASSES, commit BTC=2.4 / SOL=2.5 / LINK=2.5 as Phase 1.14 freeze. If v2.65 over-widens (NEUTRAL >25%), revert alts to 2.4 (v2.62 baseline) — that's the safe fallback that already PASSED.
