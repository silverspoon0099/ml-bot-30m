...
### 8.1 Triple-barrier parameters (v2.0 — chop-filtered ATR per Decision v2.56 / DR-007)

```yaml
labeling:
  method: triple_barrier_chop_filtered
  tp_atr_mult: 2.5                 # symmetric (tightened 3.0→2.5 per DR-007)
  sl_atr_mult: 2.5                 # symmetric
  ...
  min_atr_pct_threshold:           # CHOP FILTER (NEW per DR-007)
    BTC: 0.20                      # bars with atr_14/close × 100 < 0.20% → label = -2 (filtered)
  ...
  classes:
    ...
    SHORT: 1
    NEUTRAL: 2
    CHOP_FILTERED: -2              # NEW: bars in low-vol regime, EXCLUDED from training + inference
