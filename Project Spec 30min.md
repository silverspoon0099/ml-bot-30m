...
labeling:
  ...
  tp_atr_mult: 2.4                 # symmetric (DR-008: 2.5→2.4 per Decision v2.59)
  sl_atr_mult: 2.4                 # symmetric
  ...
  min_atr_pct_threshold:           # CHOP FILTER (per DR-007/Decision v2.56)
    BTC: 0.25                      # DR-008: 0.20→0.25 per Decision v2.59 (more selective)
    SOL: 0.30                      # alts have higher base vol; chop floor wider
    LINK: 0.30
  classes:
    LONG: 0
    SHORT: 1
...
**Param history (locked via Decisions v2.56 + v2.59 — Phase 1.11 iteration):**

| Iteration | Decision | tp/sl_atr_mult | BTC chop floor | BTC NEUTRAL | BTC chop rate | Status |
|---|---|---|---|---|---|---|
| Path D first | v2.56 (DR-007) | 2.5 | 0.20% | 33.94% | 8.06% | Both diagnostic triggers fired |
| Path D second | **v2.59 (DR-008)** | **2.4** | **0.25%** | TBD | TBD | Active |
| Path D third (if needed) | DR-009 candidate | 2.3 | 0.30% | — | — | Not triggered yet |
| Path D conservative floor | DR-009 candidate | 2.2 | 0.30% | — | — | Not triggered yet |

User professional preference (2026-05-04): incremental tightening (2.5→2.4→2.3→2.2) over single-jump (2.5→2.0) per §10.5 single-variable-change discipline. Minimum step lets each iteration's effect be measured cleanly without overshooting; chop floor moves with same incremental rhythm (0.20→0.25→0.30).
