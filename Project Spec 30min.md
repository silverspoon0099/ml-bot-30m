...
- Cross-feature: rsi vs wt divergence flag, macd vs rsi alignment (2)
- Velocity of velocity: d²rsi/dt², d²macd/dt² (4 — momentum-of-momentum is informative at 30m)
...
- ADX zone flags: trending (>25), weak (<20), accelerating (4)
...
**Drop:**
- Redundant short-period DI variants
- Fine-grained EMA crossover timing (handled better in Event Memory cat)
...
Per research §3.5, upgrade from single daily VWAP to multi-anchor architecture.

**Daily-anchored (unchanged core):**
- daily_vwap value, daily VWAP ±1σ / ±2σ bands, close vs VWAP %, close in VWAP zone (5)

**NEW anchors:**
- `swing_high_vwap_pos` — VWAP anchored at last confirmed swing high (1)
- `swing_low_vwap_pos` — VWAP anchored at last confirmed swing low (1)
- `htf_pivot_vwap_pos` — VWAP anchored at daily pivot line break (1)
- `weekly_vwap_pos` — Monday 00:00 UTC anchored VWAP (1)
- Multi-anchor confluence flag: how many anchored VWAPs agree on direction (1)
- Rolling 20-bar VWAP-of-VWAPs mean reversion distance (1)
- Distance from nearest anchored VWAP in ATR units (1)
- VWAP cross-events count last 10 bars (1)
- Close above/below "heavy" VWAP (most-touched anchor) flag (1)
...
- `pivot_position_daily_01` — continuous 0–1 position within S1–R1 range: `(close - S1) / (R1 - S1)`. Complements the discrete 0–5 zone; 0.65 means the same thing at $30k BTC or $100k BTC
- `dist_to_nearest_pivot_atr` — distance in ATR units (not just %), volatility-normalized
- `daily_pivot_weekly_pivot_confluence` — binary flag: daily P/R1/S1 within 0.25×ATR of any weekly level

**6.3 Weekly Fib-pivots (9, NEW to feature matrix):**
- Same 9 features as §6.1 but computed from weekly-reset (Monday 00:00 UTC) pivots. At 30m, 336 bars/week means weekly pivots are persistent structural levels with real tested behavior (at 5m they were 2016 bars away, too far).

**6.4 Weekly Fib-pivots — NEW encodings (2):**
- `pivot_position_weekly_01` — continuous 0–1 position within weekly S1–R1 range
- `dist_to_nearest_weekly_pivot_atr` — weekly-pivot distance in ATR units

**6.5 Swing-based Fibonacci retracements (7, NEW — separate concept from Fib-pivots):**

These are computed from the most recent confirmed swing high / swing low (fractal pivots, confirmed with the 2-left-2-right rule — already in `structure.py`). Whereas Fib-*pivots* are calculated from prior-day OHLC math, Fib-*retracements* are drawn between an observed price swing and measure where price is within that swing range. Both are useful, capturing different structure.

- `fib_retracement_pct` — continuous 0–1 scale: `(close - swing_low) / (swing_high - swing_low)`; values <0 = broke below swing low, >1 = broke above swing high
- `in_golden_pocket` — binary flag, 1 if 0.618 ≤ `fib_retracement_pct` ≤ 0.65 (highest-probability reversal zone)
- `nearest_fib_level_dist` — distance (in ATR units) to the nearest of [0.382, 0.5, 0.618, 0.786] retracement levels
- `fib_touches_382` — count of bars in last 20 that touched 0.382 within 0.1% tolerance
- `fib_touches_618` — count of bars in last 20 that touched 0.618 within 0.1% tolerance
- `extension_progress_1272` — normalized progress toward the 1.272 Fibonacci extension (for take-profit targeting in Phase 3+)
- `swing_fib_pivot_confluence` — binary flag: current Fib retracement level (within ±ATR×0.25) coincides with any daily or weekly pivot

**Look-ahead safety:** swing high/low MUST use pivots confirmed N bars ago (center bar of a fractal pivot is N bars in the past by definition), not the rolling max/min including the current bar.
...
Keep all v1.0:
- Body size %, upper wick %, lower wick %, range % (4)
- Doji flag, hammer/shooting-star flag, engulfing signal, pin bar signal, inside-bar flag (5)

30m candles carry more signal than 5m (less microstructure noise), so keeping the full set.

#### Category 9 — Mean Reversion / Statistics (8 → 7 features)

**Keep:**
- Bollinger %B, distance from mid-band in σ units, z-score of close over 20 bars, z-score over 50 bars (4)
- Rolling skew(20), kurtosis(20), autocorr(1) (3)
...
**Keep:**
- Prev-bar close, high, low relative to current close (3)
- Prev-bar volume ratio (1)
- Today's open-to-now % change (1)
- Today's high/low distance from current (1)

**Drop:**
- Yesterday's features that duplicate HTF1D category (2a already has them)

#### Category 12 — Lagged Dynamics (8 → 5 features)

**Keep:**
- Δrsi_1, Δrsi_3, Δadx_3, Δvolume_3, Δclose_vs_ema21_3 (5)

**Drop:**
- Longer lag windows that overlap with HTF context (2a already covers slower dynamics)

#### Category 13 — Divergence Detection (7 → 7, unchanged)

Keep all v1.0, signs normalized to +1 bullish / −1 bearish (the fix from v1.0 iteration):
...
- divergence_recency (bars since last divergence) (1)
...
- HH/HL/LH/LL last 20 bars (4)
- structure_type (bullish/bearish/ranging) (1)
- swing_ratio (HH count vs LL count) (1)
- last_swing_high, last_swing_low distance % (2)
- fractal_pivot_count last 20 bars (1)
- break_of_structure flag (1)

#### Category 17 — Statistical / Fractal (9 → 6 features)

**Keep:**
- Hurst exponent (100-bar window) (1)
- Fractal dimension (box-counting, 50-bar window, unit-square normalized) (1)
- Rolling autocorr(5), autocorr(20) (2)
- Rolling entropy(20) (1)
- Realized-vol-of-realized-vol (1)

**Drop:**
- Parkinson proxy (moved to Cat 3 removal list)
- Multi-scale Hurst (single scale is enough)
- Volatility-of-volatility-of-volatility and similar stackings

#### Category 18 — Adaptive MAs (5 → 4 features)

**Keep:**
- KAMA(10) position vs close % (1)
- DEMA(21) vs close % (1)
- TEMA(21) vs close % (1)
- PSAR state (+1 long / −1 short), PSAR distance % (1 aggregated field)

**Drop:**
- Duplicate KAMA parameter variants

#### Category 19 — Ichimoku (5 → 6 features)

**Keep + fix:**
- Tenkan, Kijun, Tenkan-Kijun diff % (3)
- Senkou A, Senkou B (both displaced +26 bars — v1.0 correctness fix) (2)
- cloud_dist_pct using cloud midline = (senkou_a + senkou_b)/2, sign = close-vs-cloud (1)
...
- Cat 11 Previous Context features — look only at prior bars
...
- Cat 16 market structure (HH/HL/LH/LL confirmations are static; the "current move so far" is dynamic)
- Cat 19 Ichimoku (spans displaced to past are static; Tenkan/Kijun are dynamic)
...
│   ├── pivots.py                # reused (daily + weekly already both supported)
│   ├── candles.py               # reused
│   ├── structure.py             # reused
│   ├── divergence.py            # reused (sign convention already fixed)
│   ├── event_memory.py          # MODIFIED: trim from 41→22
│   ├── adaptive_ma.py           # reused
│   ├── ichimoku.py              # reused (cloud displacement fix kept from v1.0)
│   ├── regime.py                # reused
│   ├── stats.py                 # MODIFIED: trim
...
1. **Phase-scoped selective copy + clean-glue rewrite** (per Decision Log v2.33 / DR-001, reconciled with v1.0 ground truth per Decision Log v2.34 / DR-002, and split into "kept math / fresh glue" per Decision Log v2.36 / DR-005). **§15 is the authoritative file manifest.** Each phase splits its work into three tracks: (i) *copy* algorithm files from `../ml-bot/` unchanged, (ii) *write fresh* the v2.0 glue layer from spec, (iii) create *NEW algorithm files* not in v1.0. Files not listed in §15 (5m-specific scalping features, intrabar execution code, deprecated experiments, pre-cleanup branches) stay at v1.0 and never come over. Phase scope:
   - **Phase 1 — copy from `../ml-bot/` (algorithm files: math, not glue)** per Decision v2.36 (22 files): `model/labeler.py`, `features/__init__.py`, `features/_common.py`, `features/builder.py`, `features/indicators.py`, `features/extra_momentum.py` (Cat 15), `features/volatility.py`, `features/volume.py`, `features/vwap.py`, `features/pivots.py`, `features/candles.py`, `features/structure.py`, `features/divergence.py`, `features/event_memory.py`, `features/adaptive_ma.py`, `features/ichimoku.py`, `features/regime.py`, `features/stats.py`, `features/sessions.py`, `features/context.py`, `features/ema_context.py`, `data/collectors/storage.py` (parquet I/O — no v1.0 baggage), `scripts/relabel.py`, `requirements.txt`, `.gitignore`. Plus `.env` (per step 4 below).
