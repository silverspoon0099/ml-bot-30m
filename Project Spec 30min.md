...
**Decision criterion locked upfront (NOT iterable per §10.3.1 + Decision v2.68 commitment; per-asset evaluation per Decision v2.69 / DR-016)**:

§10.3.2 walk-forward runs on **all 3 assets** (BTC + SOL + LINK; per Decision v2.69 / DR-016 extension). Each asset evaluated independently against `mean_ratio ≤ 0.98` gate. Outcomes:

- **All 3 PASS** (each asset's mean_ratio ≤ 0.98) → premise alive across full universe; 5-feat heuristic was crypto-miscalibrated; proceed to Phase 2.3 walk-forward CV with default params (per-asset training per Decision v2.35), then Phase 2.4 Optuna.
- **BTC FAILS, ≥1 alt PASSES** → architecture pivot to **tiered universe** per Decision v2.69 (DR-016). BTC dropped from v2.0 trade universe; project proceeds with passing alts only (e.g., "alt-only bot" focusing on SOL+LINK if both pass; or single-asset bot if only one passes). File DR-017 to formalize universe reduction + amend §6.2 (data volumes) + §14 (phase plan) + Appendix A (transfer learning sequence). 50% scope reduction but salvages 6 months of work; alts have known-easier-edge profile vs BTC's institutional saturation. Rationale: BTC failure on full feature set is robust evidence BTC@30m is un-extractable with our feature representation, but alt success would prove the broader signal-detection architecture is sound.
- **All 3 FAIL** (each asset's mean_ratio > 0.98) → premise robustly dead across BOTH 5-feature AND 245-feature walk-forward across the full v2.0 universe; halt v2.0 with very high confidence; **NO further "one more test"** — escalate to fresh DR re-opening §8 labeling, Phase 1.10 features, §2.1 timeframe, OR accepting v2.0 as research dead-end.
- **BTC PASSES, ≥1 alt FAILS** (rare expected — alts have higher base alpha) → diagnose per-asset divergence; likely fixable per-asset configuration; investigate before halt. Possible causes: alt label distribution off, alt feature interaction, alt-specific data quality issue.
...
| v2.69| **DR-016: Multi-asset §10.3.2 walk-forward + tiered-universe architecture option** | User strategic question 2026-05-05 ('developing a BTC-only bot would be very difficult but we are trying to develop a general bot. what 's your opinion?') surfaced a critical architecture issue with Decision v2.68 (DR-015): §10.3.2 confirmatory walk-forward was scoped to BTC ONLY, but the v2.0 universe is 3 assets (BTC + SOL + LINK). If BTC fails §10.3.2 but alts pass, the spec-strict 'halt v2.0 entirely' response throws away alt-extractable signal that we haven't yet tested for. Local-Claude analysis: BTC at 30m is one of the most efficient crypto markets (institutional ETF flows, hourly funding, ~$50B/day liquidity); alts (SOL/LINK) have documented higher base alpha at retail/institutional saturation levels. The §10.3.1 5-feat fail on BTC is consistent with this hierarchy; it does NOT generalize to alts without empirical verification. **Decision v2.69 (DR-016) LOCKED**: extend §10.3.2 confirmatory walk-forward to ALL 3 assets (BTC + SOL + LINK); per-asset PASS/FAIL evaluation; expanded decision matrix covering tiered-universe architecture option. **Per-asset gate**: each asset evaluated independently against mean_ratio ≤ 0.98 across ~25 walk-forward folds. **Expanded decision matrix**: (a) all 3 PASS → premise alive across full universe; proceed Phase 2.3 with all 3 assets per Decision v2.35 sequence; (b) BTC FAILS + ≥1 alt PASSES → **architecture pivot to tiered universe**: BTC dropped from v2.0 trade universe; project proceeds with passing alts only; file DR-017 to formalize universe reduction + amend §6.2 + §14 + Appendix A. 50% scope reduction but salvages 6 months of work; alts have known-easier-edge profile vs BTC's institutional saturation; (c) all 3 FAIL → premise robustly dead across the full v2.0 universe; halt v2.0 with very high confidence; NO further 'one more test'; escalate to fresh DR; (d) BTC PASSES + ≥1 alt FAILS (rare expected — alts have higher base alpha) → diagnose per-asset divergence; likely fixable per-asset config; investigate before halt. **Strategic rationale for tiered-universe option**: pre-Decision v2.68, the spec assumed BTC must work — Decision v2.35 sequenced BTC-first 'transfer learning' on the assumption shared features would extract shared signal. The 5-feature §10.3.1 fail challenges that assumption: BTC may need different feature representation than alts (or no extractable feature representation exists at 30m). Tiered universe acknowledges this empirically: if alts have signal but BTC doesn't, the bot becomes alt-focused; BTC unlock could be a v3.0 redesign with different timeframe/features. **Why NOT amend Phase 1 labels (e.g., tighten BTC tp/sl 2.4→2.2) before §10.3.2**: user considered this 2026-05-05 ('shall we reduce atr_mult=2.4 to 2.2 or 2.0 to increase Long/Short than NEUTRAL?'); local-Claude recommended hold-off because (i) Phase 1.14 freeze discipline; (ii) §10.3.2 is a premise check, not a label optimizer — changing labels mid-test invalidates the methodology; (iii) no diagnostic evidence yet that label distribution is the bottleneck (BTC NEUTRAL=24.03% with +9.03pp margin is mid-band, not edge); (iv) tightening operates near diminishing-returns regime per BTC ladder calibration; (v) we'd lose the ability to distinguish 'tighter labels rescued the gate' vs 'more features rescued the gate'. User agreed: 'don't tighten yet. try test on all 3 assets and then consider again if BTC only fails'. The discipline path: test current frozen artifacts on all 3 assets via §10.3.2; if BTC fails alone, file DR-017 (tiered universe) — NOT DR to tighten BTC labels (which would be tuning post-failure, the v1.0 mode the spec is designed to prevent). **Spec edits**: §10.3.2 decision-criterion paragraph expanded with 4-tier per-asset evaluation matrix; Appendix C step 2.2 wording expanded ('all 3 assets' + tiered-universe option reference); Decision v2.69 in Decision Log line 2040. Mirror synced. **No new code change beyond DR-015 scaffolding**: VPS extends `scripts/baseline_full_gate.py` (or whatever it lands as) to loop per-asset instead of BTC-only. Compute scaling: 3 assets × 25 folds × ~243-250 features × 200 LightGBM rounds × ~60-90s/fold ≈ 75-120 min total (vs ~30 min BTC-only). **Code estimate**: 30 LOC addition for asset-loop wrapper; helpers from DR-014 fully reusable. **Iteration freedom remains CLOSED on §10.3.x methodology** per Decisions v2.67 + v2.68; this DR extends asset scope, not methodology. Walk-forward stays canonical. **OOT one-shot rule (§10.4)** still does NOT apply (Phase 2.10 boundary); current iteration is on PRE-OOT data only. **DR-017 candidate (tiered-universe formalization)** stays staged but not yet locked: triggers ONLY if outcome (b) lands; otherwise unused. **ROLLBACK PROCEDURE**: revert §10.3.2 expanded decision-criterion paragraph (restore DR-015 single-asset scope); revert Appendix C step 2.2 (restore BTC-only); revert Decision Log entry. If user wants strict BTC-only §10.3.2 (per original DR-015 framing), that path remains available; user explicitly chose extended-scope DR-016 path. Approved by user 2026-05-05 with 'I agree with you that we should test all 3' confirmation | 2026-05-05 |
...
| 2.2  | Full-gate: 243/250-feature walk-forward baseline on **ALL 3 ASSETS** (BTC + SOL + LINK; per-asset evaluation; mean val log-loss ≥2% better than mean prior across ~25 folds per §10.3.2 amended by Decisions v2.68 + v2.69; runs out-of-sequence as confirmatory premise check after §10.3.1 walk-forward FAIL on 2026-05-05). Decision matrix per §10.3.2: all 3 PASS → Phase 2.3 full universe; BTC fails + alts pass → tiered-universe architecture (DR-017 candidate); all 3 fail → halt v2.0 | §10.3.2 + §10.3.2.tiered (Decision v2.69) | ⏳ DR-016 multi-asset work order issued 2026-05-05 |
