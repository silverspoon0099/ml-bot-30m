...
labeling:
  ...
  tp_atr_mult:                     # DR-011: PER-ASSET dict per Decision v2.64 (was scalar 2.4 in v2.61)
    BTC: 2.4                       # frozen at v2.61 PASS state — do not perturb
    SOL: 2.2                       # DR-011: 2.4→2.2 alts-only experiment (NEUTRAL-floor risk; see Decision v2.64)
    LINK: 2.2                      # same as SOL initially; differentiate in DR-012 if needed
  sl_atr_mult:                     # PER-ASSET dict (symmetric with tp)
    BTC: 2.4                       # frozen at v2.61 PASS
    SOL: 2.2                       # DR-011: 2.4→2.2 alts-only
    LINK: 2.2
...
| Path D fifth — per-asset tp/sl experiment | **v2.64 (DR-011)** | **BTC=2.4 / SOL=2.2 / LINK=2.2** | **0.275% BTC / 0.60% SOL+LINK** | **12** | (BTC unchanged) | (BTC unchanged) | **Active for SOL+LINK; BTC frozen at v2.61 PASS state — DO NOT RE-RUN** |
| Fallback if v2.64 NEUTRAL breach | DR-012 candidate (revert) | revert SOL/LINK to 2.4 | (asset-specific) | 12 | — | — | Triggers if NEUTRAL <13% on alts post-2.2 |
| Fallback if v2.64 succeeds + extend | DR-012 candidate (also BTC) | system 2.2 | (asset-specific) | 12 | — | — | Triggers if 2.2 wins clearly on alts |
...
- v2.64 (DR-011) is the per-asset tp/sl experiment. v2.62 result: BTC + SOL + LINK ALL PASSED §8.2 bands (NEUTRAL 24.03% / 15.55% / 15.75%). Alts at NEUTRAL floor edge (15-15.75% just above 15% target). User professional hypothesis: tightening tp/sl on alts (2.4 → 2.2) before Phase 1.14 freeze might increase directional sample density without breaching NEUTRAL floor — potentially better Phase 2 signal density via reduced label-class imbalance. **Schema extension**: §8.1 `tp_atr_mult` and `sl_atr_mult` migrate from scalar to per-asset dict (matching existing pattern for `min_profit_pct` + `min_atr_pct_threshold`). BTC frozen at 2.4 (v2.61 PASS state preserved); SOL + LINK to 2.2. **Empirical projection from BTC iteration ladder (v2.56→v2.59 = −1.80pp NEUTRAL per 0.1 ATR step)**: alts NEUTRAL likely drops 15.55%→13.7%, 15.75%→13.9% — both projected to BREACH 15% target floor (but stay above 10% hard-emergency floor). Outcome judgment deferred to post-test review: §8.2 band is quality target not hard constraint per spec wording; LightGBM tolerates 13-14% NEUTRAL; user assesses Phase 2 sample-density vs band-discipline trade-off after the data lands.
